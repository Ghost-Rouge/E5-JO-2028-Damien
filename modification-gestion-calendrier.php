...
$sql = "SELECT e.*, l.nom_lieu, s.nom_sport 
        FROM EPREUVE e
...
<header>
    ...
    <?php foreach ($epreuves as $epreuve): ?>
        <tr>
            ...
            <td data-label='Date:'>
                <?php
                    $date = DateTime::createFromFormat('Y-m-d', $epreuve['date_epreuve']);
                    echo $date ? $date->format('d/m/Y') : htmlspecialchars($epreuve['date_epreuve'], ENT_QUOTES, 'UTF-8');
                ?>
            </td>
            <td data-label='Heure:'>
                <?php
                    $heure = DateTime::createFromFormat('H:i:s', $epreuve['heure_epreuve']);
                    echo $heure ? $heure->format('H:i') : htmlspecialchars($epreuve['heure_epreuve'], ENT_QUOTES, 'UTF-8');
                ?>
            </td>
            <td data-label='Lieu:'><?= htmlspecialchars($epreuve['nom_lieu'], ENT_QUOTES, 'UTF-8') ?></td>
            <td data-label='Sport:'><?= htmlspecialchars($epreuve['nom_sport'], ENT_QUOTES, 'UTF-8') ?></td>
            <td>
